...
void gravity(vector<vector<char>>&);
    ...
    do
    {
        ...
        do
        {
            ...
            else if (choice == 'w' && playerRow != 0)
            {
                ...
                ///possible the gnarliest and coolest one liner I will ever write...
                if(arr[playerRow+1][playerCol] == '#') for (int i = playerRow, jmpHeight = 3; (i > 0) && (arr[i-1][playerCol] !=  '#') && (jmpHeight > 0); i--, jmpHeight--, playerRow-- );

                ///an explanation/equivalent form of the original form
//                //check if the player is on the ground (later I will add a double jump option)
//                if(arr[playerRow+1][playerCol] == '#')
//                {
//                    for (int i = playerRow, jmpHeight = 3;  ///jump height determines how high to jump
//                            (i > 0) &&                      ///you should not continuing moving up if you reach the ceiling of the map
//                            (arr[i-1][playerCol] !=  '#') &&///you should not jump thru floor bricks
//                            (jmpHeight > 0); )              ///check that we have not depleted our jump height
//                    {
//                        i--;
//                        jmpHeight--;
//                        playerRow--;
//                    }
//                }
                ///I noticed the one line braceless variant after writing the braced version! :D
                goodInput = true;
            }
...
void gravity(vector<vector<char>>& arr)
{
    int prevRow = playerRow;
    int prevCol = playerCol;
    arr[playerRow][playerCol] = ' ';
    ...
    if(arr[playerRow+1][playerCol] == '#')
    {
        ;   ///don't do anything if we are on the ground
    }
    else if (playerRow == ROWS - 1)
    {
        ;   ///don't do anything if we are at the end
    }
    else
    {
        playerRow++;
