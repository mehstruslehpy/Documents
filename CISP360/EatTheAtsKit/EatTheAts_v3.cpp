...
int moveMonsters(char[ROWS][COLS]);
///move a single monster on the board
int moveMonster(char[ROWS][COLS], int, int, int, int&);
...
void dispBoard(char arr[ROWS][COLS])
{
    ...
    ///system cls causes flickering depending on the refresh rate.
    ///system("cls");
    ClearScreen();
    ///refreshBoard();
...
int moveMonsters(char arr[ROWS][COLS])
{
    int eatenFlag = 0;
...
int moveMonster(char arr[ROWS][COLS], int row, int col, int direction, int &eatenflag)
{
    if (direction == -1)
    {
        return -1;
