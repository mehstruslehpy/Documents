...
(defun player-attack ()
  ...
  (case (read)
	...
	(r (dotimes (x (1+ (randval (truncate (/ *player-strength* 3)))))		 ;do a roundhouse attack on a random monster 
			    (fresh-line)
				(unless (monsters-dead)
				   (monster-hit (random-monster) 1))))
	(t (princ "You chose nothing!")(player-attack)))
	(fresh-line))															 ;if the user picks nothing prompt them for another input recursively
...
(defun pick-monster ()												
  ...
  (let ((x (read)))												;read an input
    (if (not (and (integerp x) (>= x 1) (<= x *monsters-num*)))	;if not (an integer & >= 1 & <= *monsters-num*) 
        ...
        (map 'vector												;map to a vector
		     (lambda (x)											;the lambda function
			   (funcall (nth (random (length *monster-builders*))	;funcall a random function from *monster-builders*
			   			*monster-builders*)))
			 (make-array *monsters-num*))))							;on each value of the array
