...
enum { LVAL_ERR, LVAL_DEC, LVAL_NUM, LVAL_SYM, LVAL_SEXPR };
...
lval* builtin_op(lval* a, char *op ) {
	...
	for (int i = 0; i < a->count; i++) {
		if (a->cell[i]->type != LVAL_NUM && a->cell[i]->type != LVAL_DEC) {
			lval_del(a);
			return lval_err("CANNOT OPERATE ON A NON NUMBER OR DECIMAL!");	
		}
	}
	...
	while (a->count  > 0) {
		...
		if (x->type == LVAL_NUM && y->type == LVAL_NUM) {		
			...
			if (strcmp(op, "-") == 0) { x->dec -= y->dec; }
			if (strcmp(op, "%") == 0) { x->dec = (long) x->dec % (long) y->dec; }
			if (strcmp(op, "*") == 0) { x->dec *= y->dec; }
			if (strcmp(op, "/") == 0) { 
				if (y->dec == 0) {
					lval_del(x); lval_del(y);
...
lval* lval_read_dec(mpc_ast_t* t) {
	errno = 0;
	/* plus one to remove leading d*/
	double x = strtof(t->contents + 1, NULL);
	return errno != ERANGE ?
...
lval* lval_read_num(mpc_ast_t* t) {
	errno = 0;
	/* plus one to remove leading i*/
	long x = strtod(t->contents + 1, NULL);
	return errno != ERANGE ?
...
int main(int argc, char** argv) {
	...
	mpc_cleanup(5, Decimal, Number, Symbol, Sexpr, Expr, Lispy);
	return 0;
}
