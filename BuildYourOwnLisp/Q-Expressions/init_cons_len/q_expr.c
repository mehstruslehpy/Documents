...
void lval_del(lval* v) {
	switch (v->type) {
		...
		case LVAL_SEXPR:
			...
			//also free the memory allocated to contain pointers
			free(v->cell);
			break;
	}
	free(v);
...
lval* lval_pop(lval* v, int i) {
	lval* x = v->cell[i];
	//the number of lvals, minus the element to be popped, minus one; equals bytes to move by??
