	...
	switch (lookahead) {
		...
		case ID:
			...
			if (lookahead == 58) {
				...
				printf("LVALUE '%s'        ;push address of data location %s \n", symtable[offset].lexptr, symtable[offset].lexptr);	
				expr();
				symtable[offset].offset = varpos - 1;						//ensures counting always starts from 1
				printf("ASSIGN        ;':=' pops the rvalue and the lvalue below it from the top of the stack, the lvalue now refers to that rvalue \n\n");	
